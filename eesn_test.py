...
if __name__ == '__main__':
    ...
    EXPERIMENT_NAME = "TEST_N_RESERVOIRS_"

    res_ranges = [(10, 300)]
    echo_ranges = [(0.8, 0.8)]
    spect_ranges = [(1.25, 1.25)]
    res_number_ranges = [5, 10, 20, 30, 40, 50, 60]
    # num_res = 20
    reg = 1e-4

    num_samples = 20

    data_samples = np.zeros((len(res_ranges)*len(echo_ranges)*len(spect_ranges)*len(res_number_ranges), 1+2+2+2+1+1))
    data_csv = np.zeros((len(res_ranges)*len(echo_ranges)*len(spect_ranges)*len(res_number_ranges), 1+2+2+2+1+1))
